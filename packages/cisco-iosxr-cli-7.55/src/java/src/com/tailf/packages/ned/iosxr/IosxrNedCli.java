...
import static com.tailf.packages.ned.nedcom.NedString.kpKeyQuote;
 ...
 *
 */
@SuppressWarnings( {"deprecation", "squid:S135", "squid:S1172", "squid:S3776", "squid:ForLoopCounterChangedCheck"} )
public class IosxrNedCli extends NedComCliBase {
    ...
    private static final String QMARK_TRICK = "\\?\u0002\u0008\u0005";     // unicode: ctrl-b,back space,ctrl-e
    ...
    private static final Pattern[] NOPRINTLINEWAIT_PATTERN = new Pattern[] {
    ...
    private static final String[] staticWarning = {
        "warning[:] policy object route[-]policy \\S+ exists"
    };
    ...
    private String showVersion = UNKNOWN;
    ...
    private boolean showHiddenInterfaces;
    ...
    public void logInfo(NedWorker worker, String msg) {
        log.info(msg);
    }
    public void logVerbose(NedWorker worker, String msg) {
        log.verbose(msg);
    }
    public void logDebug(NedWorker worker, String msg) {
        log.debug(msg);
    }
    ...
    protected void setupParserContext(Schema.ParserContext pctx) {
        ...
        if (apiEditBanner) {
            traceVerbose(worker, "adding /tailfned/api/edit-banner");
            pctx.addVirtualLeaf("/tailfned/api/edit-banner", "");
        }
        if (apiSpList) {
    ...
    public void nedSettingsDidChange(NedWorker worker, Set<String> changedKeys, boolean isConnected) throws Exception {
        ...
        // Cache ned-settings
        try {
            final String parser = nedSettings.getString(NedSettings.EXTENDED_PARSER, "auto");
            if ("robust-mode".equals(parser) || "disabled".equals(parser)) {
                throw new NedException("ned-settings: extended-parser "+parser+" is deprecated, please change value");
            }
            ...
            showHiddenInterfaces = nedSettings.getBoolean("read/show-hidden-interfaces", false);
            stripDescriptions = nedSettings.getBoolean("read/strip-descriptions", false);
            ...
            writeChunkSize = nedSettings.getInt("write/number-of-lines-to-send-in-chunk");
            ...
            apiEditBanner = nedSettings.getBoolean("api/edit-banner");
    ...
    protected PlatformInfo setupDevice(NedWorker worker) throws Exception {
        ...
        try {
            ...
            printLineExec(worker, "terminal length 0");
            printLineExec(worker, "terminal width 0");

            // Show version
            String version = printLineExec(worker, "show version brief");
            if (isExecError(version)) {
                version = printLineExec(worker, "show version");
            }
            ...
            if (version.contains(" NETSIM")) {
                ...
                printLineExec(worker, "show confd-state version");
                printLineExec(worker, "show confd-state loaded-data-models data-model tailf-ned-cisco-ios-xr");

                // Set NETSIM data and defaults
                readTransIdMethod = "config-hash"; // override
            ...
            //
                ...
                if (platformModelRegex != null) {
                    ...
                    if (m.find()) {
                        this.iosmodel = m.group(1);
                    } else if ((match = getMatch(version, "\ncisco (\\S+) processor")) != null) {
                        this.iosmodel = match;
                    } else {
                        p = Pattern.compile("\n(NCS.+)");
                        m = p.matcher(version);
                ...
                if (logIsVerbose()) {
                    printLineExec(worker, "show install active summary");
                }
            }
    ...
    private String getDeviceProfile(int th) {
        try {
            String p = "/ncs:devices/device{"+kpKeyQuote(this.device_id)+"}/device-profile";
    ...
    private String getSerialDiag(NedWorker worker) throws IOException, SSHSessionException {
        ...
        String serial = printLineExec(worker, "show diag | include S/N");
    ...
    private String getSerialInventory(NedWorker worker) throws IOException, SSHSessionException {
        final long perf = tick(0);
        String serial = printLineExec(worker, "show inventory | include SN");
        ...
        if (session != null) {
            ...
            if ("sftp-transfer".equals(readDeviceMethod) && readDeviceFile != null) {
                ...
                if (path != null) {
                    ...
                    String reply = printLineExec(worker, "dir "+path);
    ...
    private String getConfig(NedWorker worker, boolean convert) throws Exception {
        ...
        try {
            ...
            // devices device <dev> live-status exec any sync-from-file <file>
                ...
                res = printLineExec(worker, "file show " + syncFile);
            ...
            if (isDevice() && session != null && showHiddenInterfaces) {
                res = getAllInterfaces(worker) + res;
            }
    ...
    private String getAllInterfaces(NedWorker worker) throws IOException, SSHSessionException {
        String res = printLineExec(worker, "show interface all | inc \"line proto\"");
        String[] lines = res.split("\n");
        StringBuilder interfaces = new StringBuilder();
        for (String line : lines) {
            if (!line.contains("line proto")) {
                continue;
            }
            String[] tokens = line.split(" ");
            if (tokens.length > 0) {
                interfaces.append("interface ");
                interfaces.append(tokens[0].trim());
                interfaces.append("\n!\n");
            }
        }
    ...
    private String getAdminConfig(NedWorker worker, boolean enterAdmin)
        ...
        String res = "";
        try {
            res = printLineExec(worker, "show running-config");
            if (isExecError(res)) {
    ...
    private NedSSHClient sftpConnect(NedWorker worker) throws NedException {
        ...
        try {
            ...
            logError(worker, "error in sftp connect", e);
    ...
    private String sftpGetConfig(NedWorker worker) throws Exception {
        ...
        printLineExec(worker, "delete /noprompt " + readDeviceFile);
     ...
     * @throws NedException
     */
    private String modifyInput(NedWorker worker, String res)
    ...
    private String modifyInputGroup(NedWorker worker, String res, String groupName)
        ...
        if (isDevice) {
            ...
            while (m.find()) {
                ...
                final String marker = passwordQuote(m.group(2));
                final String message = m.group(3);

                //
                // Append lines numbers in front of banner content lines
                //
                if (apiEditBanner) {
                    traceVerbose(worker, "transformed <= quoted edit-banner "+name);
                    sbret.append("banner "+name+"\n");
                    String[] blines = message.replace("\r", "").split("\n");
                    ConfPath cp = new ConfPath(operRoot+"/edit-banner-list{\""+name+"\"}");
                    String[] lineno = null;
                    ...
                    int indx = 0;
                    int num = 0;
                    for (int b = 0; b < blines.length; b++) {
                        if (blines.length > 1 && b == 0 && blines[b].trim().isEmpty()) {
                            continue;
                        }
                        ...
                        sbret.append(Integer.toString(num)+" \""+blines[b]+"\"\n");
                    }
        ...
        for (n = 0; n < lines.length; n++) {
            ...
            if ((i = line.indexOf(" description ")) >= 0) {
                if (toptag.startsWith("l2vpn")
                    ...
                    || toptag.startsWith("banner")
                    || toptag.startsWith("group ")
                    || "description".equals(line.substring(i).trim()) // no description string
                    || (i > 1 && line.charAt(i-1) == '#')) {
                    // Ignore '# description' entries in e.g. route-policy or sets
                    sbin.append(line+"\n");
                } else {
                    // Quote ' description ' strings
                    String desc = line.substring(i+13);
                    if (isNetsim()) {
                        desc = desc.trim();
                    } else {
                        if (desc.charAt(desc.length() - 1) == '\r') {
                            desc = desc.substring(0, desc.length()-1);
                        }
                        if (stripDescriptions && (desc.charAt(desc.length() - 1) == ' ')) {
                            desc = desc.substring(0, desc.length()-1);
                        }
                        // Trim NSO strange spacing
                        int si = 0;
                        while (desc.charAt(si) == ' ') {
                            si++;
                        }
                        desc = desc.substring(si);
                    }
                    desc = stringQuote(desc);
        ...
        for (n = 0; n < lines.length; prevline = lines[n], n++) {
            ...
            //
                ...
                if (apiEditRoutePolicy) {
                    ...
                    ConfPath cp = new ConfPath(operRoot+"/edit-list{\""+name+"\"}");
                    String[] lineno = null;
                    if (cdbOper.exists(cp)) {
                        String val = ConfValue.getStringByValue(cp, cdbOper.getElem(cp.append("/lineno")));
                        lineno = val.split(" ");
        ...
        // Done
        return sbin.toString() + sbret.toString();
    }
    ...
    private String injectInput(NedWorker worker, String res) {
        ...
        if (apiEditBanner) {
            traceInfo(worker, "transformed <= inserted 'tailfned api edit-banner'");
            first.append("tailfned api edit-banner\n");
        }
        if (apiSpList) {
    ...
    public void showPartial(NedWorker worker, String[] cp) throws Exception {
        ...
        if (isNetsim()) {
            ...
            for (String cmdPath : cmdPaths) {
                ...
                String dump = printLineExec(worker, show);
        ...
        // Upload device config to NSO using turbo-parser
        parseAndLoadXMLConfigStream(maapi, worker, schema, config);
    ...
    private String doShowPartial(NedWorker worker, String cmdPath)
        ...
        if (cmdPath.trim().equals("admin")) {
            ...
            dump = printLineExec(worker, show);
        }
    ...
    private String doGetTransId(NedWorker worker, String res) throws Exception {
        ...
        //
        if (!adminLogin && "commit-list".equals(readTransIdMethod)) {
    ...
    private String getCommitId(NedWorker worker) throws NedException, IOException, SSHSessionException {
        ...
        // First try showing only 1 (or 10) commit ids (to be able to filter out the MIBD entries below)
        boolean doFilterMIBD = (apiSnmpServerEnableAllTraps != 0);
        String res = printLineExec(worker,
                                     String.format("show configuration commit list %d", doFilterMIBD ? 10 : 1));
        ...
        if (!isExecError(res)) {
            ...
            if (i > 0) {
                ...
                for (int n = 1; n < lines.length; n++) {
                    ...
                    if (doFilterMIBD && "MIBD".equals(column[4])) {
        ...
        res = printLineExec(worker, "show configuration commit list");
        if (isExecError(res)) {
        ...
        if (res.contains("Line") && ((res = getMatch(res, "\n(?:\\d+) +(\\d+) ")) != null)) {
            // Above failed (possibly < 10 commits done + MIBD filtering)
            return res;
        }
        if ((res = getMatch(res, "\n(\\d+) ")) != null) {
            // Admin mode on NCS
    ...
    public void prepareDry(NedWorker worker, String data) throws Exception {
        ...
        try {
            ...
            for (int n = 0; n < lines.length; n++) {
                String line = lines[n];
                if (line.trim().startsWith("ios-regex ")) {
                    line = line.replace(QMARK_TRICK, "?");
                }
                sb.append(line+"\n");
            }
    ...
    private void doApplyConfig(NedWorker worker, String data)
        ...
        // commit show-error
        if (iosversion.startsWith("4") || !isDevice() || adminLogin) {
        ...
        try {
            ...
            if (!data.trim().isEmpty()) {
                ...
                sendConfig(worker, data, true, writeChunkSize);
                // In config mode
            }
            ...
            if (!session.serverSideClosed()) {
                if (reply.contains("Failed to commit") && reply.contains("show configuration failed")) {
                    reply = printLineExec(worker, "show configuration failed");
                }
                session.print("abort\n");
            }
            inConfig = false;
    ...
    private String commitAdminConfig(NedWorker worker, String data)
        ...
        // Enter config mode and send admin config. Enforce non-chunk mode due to problems with aaa config [IOSXR-1453]
        sendConfig(worker, adminData, false, 1);

        // Commit admin config
        printLineWaitOper(worker, "commit", writeTimeout);
        numAdminCommit++;
     ...
     * @throws Exception
     */
    private void sendConfig(NedWorker worker, String data, boolean useSftp, int chunkSize)
        throws NedException, IOException, SSHSessionException, ApplyException {
        ...
        try {
            ...
            if (useSftp && lines.length >= writeSftpThreshold) {
                ...
                doSendConfig(worker, lines, chunkSize);
            }
     ...
     * @throws Exception
     */
    private void doSendConfig(NedWorker worker, String[] lines, int chunkSize)
        throws NedException, IOException, SSHSessionException, ApplyException {
                ...
                for (; n < lines.length; n++) {
                    ...
                    if (trimmed.startsWith("description ")) {
                        if (isNetsim()) {
                            ...
                            // Only trim space before description (i.e. for 30152), but keep trailing (for RT48854)
                            int i = 0;
                            while (line.charAt(i) == ' ') {
                                i++;
                            }
    ...
    private String[] modifyOutput(NedWorker worker, String data, int fromTh, int toTh, String function)
        ...
        try {
            ...
            // Single string group:
            if (!apiGroupModeled) {
                lines = modifyOutputGroup(worker, "top", data, fromTh, toTh);
            }
            ...
            // Detailed groups:
                ...
                for (String dump : parts) {
                    ...
                    if (!dump.startsWith("group ")) {
                        this.confPath = this.confRoot;
                        lines = modifyOutputGroup(worker, "top", dump, fromTh, toTh);
                        for (String line : lines) {
                            linesList.add(line);
                        }
                        continue;
                    ...
                    lines = modifyOutputGroup(worker, group, sbc.toString(), fromTh, toTh);
     ...
     * @throws NedException
     */
    private String[] modifyOutputGroup(NedWorker worker, String groupname, String data, int fromTh, int toTh)
        throws NedException {

        if (data.trim().isEmpty()) {
            traceDev(worker, groupname+" group empty data, leaving");
            return data.trim().split("\n");
        }

        //
        // api edit-route-policy ned-setting
        //
        if (apiEditRoutePolicy) {
            if (isDevice()) {
                data = editOutputRoutePolicyData(worker, data, toTh);
            } else if (!this.isDry && getMatch(data, "(route-policy \\S+\n \\d+ )") != null) {
        ...
        for (int n = 0; n < lines.length; n++) {
            ...
            if (isNetsim()) {
                ...
                // alias patch
                    ...
                    String alias = group[2].replace("\"", "\\\""); // Convert " to \"
        ...
        for (int n = 0; n < lines.length; n++) {
            ...
            // Preserve \n in single "line" send trick
            lines[n] = lines[n].replace("<XYZ-LF>", "\n");

            //
            // banner <type> <marker> "<message>" <marker>
            //
            if (!apiEditBanner && trimmed.startsWith("banner ")) {
                ...
                if (m.find()) {
                    ...
                    // Note: Send banner as a single line to device
    ...
    /**
     * Edit output route-policy data
     * @param
     * @return
     * @throws NedException
     */
    private String editOutputRoutePolicyData(NedWorker worker, String data, int toTh) throws NedException {
        ...
        try {
            ...
            for (; n < lines.length; n++) {
                ...
                if (line.startsWith("route-policy ")) {
                    ...
                    String lineno = editListToStringBuilder(worker, path, toTh, sb, false);
                    if (!lineno.isEmpty()) {
                        name = line.replace("\"", "");
                        ConfPath cp = new ConfPath(operRoot+"/edit-list{\""+name+"\"}");
                        if (!cdbOper.exists(cp)) {
            ...
            throw new NedException("editoutputroutepolicydata '"+lines[n]+"' error: "+e.getMessage(), e);
        }
        return data;
    }
    ...
    private String modifyOutputSets(NedWorker worker, String data, int toTh) throws NedException {
        ...
        try {
            ...
            throw new NedException("modifyOutputsets '"+lines[n]+"' error: "+e.getMessage(), e);
        }
        return data;
    }
     ...
     * @throws Exceptions
     */
    private void noPrintLineWait(NedWorker worker, String line)
        throws NedException, IOException, SSHSessionException, ApplyException {

        traceDev(worker, "noPrintLineWait() - line: "+stringQuote(line));

        final String trimmed = line.trim();

        // (1) - Expect the echo of the line(s) we sent
        if (apiEditBanner && trimmed.startsWith("banner ")) {
            traceDev(worker, "noPrintLineWait() - ignored echo line(s) for: "+stringQuote(line));
        } else {
            for (String wait: trimmed.split("\n")) {
                if (!wait.trim().startsWith("ios-regex ")) {
                    traceDev(worker, "noPrintLineWait() - waiting for echo: "+stringQuote(wait));
                    session.expect(new String[] { Pattern.quote(wait) }, worker);
                    traceDev(worker, "noPrintLineWait() - got echo: "+stringQuote(wait));
                }
            }
        }
        ...
        //traceDev(worker, "noPrintLineWait() - waiting for prompt");
        NedExpectResult res;
        try {
            res = session.expect(NOPRINTLINEWAIT_PATTERN, worker);
        } catch (Exception e) {
            if (session.serverSideClosed()) {
        ...
        String match = expectGetMatch(res);
        traceDev(worker, "noPrintLineWait() - prompt matched("+res.getHit()+"): "+stringQuote(match));
        switch (res.getHit()) {
    ...
    private boolean isCliError(NedWorker worker, String reply) {
        ...
        for (int n = 0; n < dynamicWarning.size(); n++) {
            if (findString(dynamicWarning.get(n), reply) >= 0) {
                traceInfo(worker, "ignoring dynamic warning: "+stringQuote(reply));
                return false;
            }
        }

        // Ignore static warnings [case insensitive]
        for (int n = 0; n < staticWarning.length; n++) {
            if (findString(staticWarning[n], reply.toLowerCase()) >= 0) {
                traceInfo(worker, "ignoring static warning: "+stringQuote(reply));
                return false;
            }
        }
        ...
        for (int n = 0; n < staticErrors.length; n++) {
            if (reply.contains(staticErrors[n])) {
                traceInfo(worker, "");
                traceDev(worker, "matched error: "+stringQuote(staticErrors[n]));
                return true;
            }
        }
     ...
     * @throws Exceptions
     */
    private void printLineWaitOper(NedWorker worker, String line, int timeout)
        throws SSHSessionException, IOException, NedException {
        resetTimeout(worker, timeout, 0);
        printLineWaitOper0(worker, line, timeout);
        setReadTimeout(worker);
    }
     ...
     * @throws Exception
     */
    private void printLineWaitOper(NedWorker worker, String line)
        throws IOException, SSHSessionException, NedException {
        printLineWaitOper0(worker, line, 0);
    }
     ...
     * @throws Exceptions
     */
    private void printLineWaitOper0(NedWorker worker, String line, int timeout)
        throws IOException, SSHSessionException, NedException {
        ...
        for (int retry = writeOobExclusiveRetries; retry >= 0; retry--) {
            ...
            // "Do you wish to proceed with this commit anyway?"
                ...
                if ("no".equals(commitOverrideChanges)) {
                    printLineExec(worker, "no");
                    String msg0 = printLineExec(worker, "show configuration history last 3");
                    String msg1 = printLineExec(worker, "show configuration commit changes last 1");
                ...
                printLineExec(worker, "show configuration history last 5");
            }
            ...
            if (!line.trim().startsWith("commit")) {
                ...
                if (reply.contains("Please use the command 'show configuration failed rollback")) {
                    // rollback failed
                    String msg = printLineExec(worker, "show configuration failed rollback");
            ...
            if (reply.contains("issue") && reply.contains("show configuration failed")) {
                String msg = printLineExec(worker, "show configuration failed");
                if (!msg.contains("No such configuration")) {
                    throw new NedException("\n'"+line+"' command failed:\n"+msg);
                }
            }
    ...
    protected void exitAdmin(NedWorker worker) throws IOException, SSHSessionException {
        if (adminLogin) {
            traceVerbose(worker, "admin login, ignoring admin mode exit");
            return;
        }
        if (!readAdminShowRun) {
            traceVerbose(worker, "admin show is (auto?) disabled, ignoring admin mode exit");
            return;
        }

        traceVerbose(worker, "exiting admin mode");
        printLineExec(worker, "exit");
     ...
     * @throws NedException
     */
    protected String editListToStringBuilder(NedWorker worker, String path, int th, StringBuilder sb, boolean ext)
        throws NedException {
    ...
    private int setListToStringBuilder(NedWorker worker, String path, int th, StringBuilder sb)
        ...
        try {
            ...
            for (int n = 0; n < list.size(); n++) {
                ...
                if (value.startsWith("ios-regex ")) {
                    value = value.replace("?", QMARK_TRICK);
                }
                if ((s + 1 < list.size()) && (value.charAt(0) != '#')) {
                    sb.append(" "+value+",\n");
                } else {
                    sb.append(" "+value+"\n");
                }
     ...
     * @throws NedException
     */
    private String injectOutputData(NedWorker worker, String data, String[] entry, String dir) {
    ...
    private void sftpPutConfig(NedWorker worker, String data)
        ...
        printLineExec(worker, "do delete /noprompt "+writeDeviceFile);
        ...
        try {
            client = sftpConnect(worker);
            NedSSHClient.SecureFileTransfer sftp = client.createSFTP();
            if (nedSettings.getBoolean("connection/ssh/sshj-force-legacy-sftp", false)) {
                sftp.setProtocolVersion(2);
                sftp.setPreserveAttributes(false);
            }
        ...
        String res = printLineExec(worker, "load "+writeDeviceFile);
        ...
        if (res.contains("Syntax/Authorization errors in one or more commands")) {
            res = printLineExec(worker, "show configuration failed load");
    ...
    private void doCommit(NedWorker worker, boolean allowTrial) throws Exception {
        ...
        try {
            ...
            if (allowTrial && "confirmed".equals(commitMethod)) {
                ...
                final long start = tick(0);
                printLineWaitOper(worker, cmd, writeTimeout);
                pendingTrial = true;
                traceInfo(worker, "device trial commit completed in "+String.format("%d milliseconds", tick(start)));
            ...
            // (confirm) commit
            final long start = tick(0);
            printLineWaitOper(worker, commitCommand, writeTimeout);
            pendingTrial = false;
            traceInfo(worker, "device commit completed in "+String.format("%d milliseconds", tick(start)));
            numCommit++;
    ...
    private void doRollback(NedWorker worker) throws Exception {
        ...
        try {
            ...
            if (inConfig) {
                ...
                printLineWaitOper(worker, "abort");
                inConfig = false;
            }
            ...
            if (numAdminCommit > 0) {
                try {
                    ...
                    enterAdmin(worker);
                    String res = printLineExec(worker, "rollback configuration last "+numAdminCommit, writeTimeout);
                    if (isExecError(res)) {
                        enterConfig(worker);
                        printLineWaitOper(worker, "rollback configuration", writeTimeout);
            ...
            if (numCommit > 0) {
                try {
                    ...
                    if (isNetsim()) {
                        enterConfig(worker);
                        printLineWaitOper(worker, "rollback configuration "+(numCommit-1), writeTimeout);
                        doCommit(worker, false);
                    } else {
                        printLineWaitOper(worker, "rollback configuration last "+numCommit, writeTimeout);
                    }
                } finally {
                    numCommit = 0;
                }
    ...
    private int internalCmdCheckConfigTrace(NedWorker worker, File file, StringBuilder reply, StringBuilder uniqueSb)
        ...
        if (sb.indexOf("Building configuration...") < 0) {
            reply.append("\n+++ "+name+":\n  Model: "+model+" Version: "
    ...
    public boolean keepAlive(NedWorker worker) {
        ...
        try {
            ...
            alive = false;
            logError(worker, "keep-alive error: "+e.getMessage(), e);
        }
     ...
     */
    private String printLineExec(NedWorker worker, String line) throws IOException, SSHSessionException {
     ...
     * @throws IOException, SSHSessionException
     */
    private String printLineExec(NedWorker worker, String line, int timeout)
        throws IOException, SSHSessionException {
    ...
    public boolean isDevice() {
        if (this.showVersion.contains("CUSTOMNETSIM")) {
            return false;
        }
        return !super.isNetsim();
    }

    @Override
    public boolean isNetsim() {
        if (this.showVersion.contains(" NETSIM")) {
            return true;
        }
        return super.isNetsim();
    }
     ...
     * @return
     */
    protected String textDequote(String aText) {
