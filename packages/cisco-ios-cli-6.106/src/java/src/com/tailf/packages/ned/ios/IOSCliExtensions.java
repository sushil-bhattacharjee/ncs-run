...
public class IOSCliExtensions implements NedComCliBase.ExtensionsHandler {
    ...
    private HashMap<String,String> readBoolean;
    private ArrayList<String[]> operLockSet;
    private ArrayList<String[]> operBooleanSet;
    ...
    private boolean devDisableConfigLocks;
    private boolean autoDisableConfigLockIfSp;
    ...
    public IOSCliExtensions(IOSNedCli owner) {
        ...
        this.devDisableConfigLocks = owner.nedSettings.getBoolean("developer/disable-config-locks", false);
        this.autoDisableConfigLockIfSp = owner.nedSettings.getBoolean("auto/disable-config-lock-if-sp", false);

        owner.logVerbose(null, "ioscliextensions created");
    }
    ...
    public void initialize() {
        ...
        this.readBoolean = new HashMap<>();
        this.operLockSet = new ArrayList<>();
        this.operBooleanSet = new ArrayList<>();
    }
     ...
     */
    @Override
    public void done(NedWorker worker, Schema.ParserContext pctx) {

        final String dir = (pctx.parserDirection == Schema.ParserDirection.TO_DEVICE ? "to" : "from");
        traceInfo(worker, "");
        traceInfo(worker, "begin extension exit handler ("+dir+"-device)");

        final long start = owner.tick(0);
        final int locks = operLockSet.size();
        final int booleans = operBooleanSet.size();
        try {
            //
            // direction - both
            //
            if (locks + booleans > 0) {
                ...
                int th = owner.maapi.startTrans(Conf.DB_OPERATIONAL, Conf.MODE_READ_WRITE);

                // Cache locks in oper data
                if (locks > 0) {
                    traceInfo(worker, "locks: adding|updating "+locks+" entries");
                    for (int i = 0; i < locks; i++) {
                        final String[] entry = operLockSet.get(i);
                        final String lockOperPath = entry[0];
                        if (!owner.maapi.exists(th, lockOperPath)) {
                            owner.maapi.create(th, lockOperPath);
                        }
                        owner.maapi.setElem(th, new ConfBuf(entry[1]), lockOperPath+"/path");
                        owner.maapi.setElem(th, new ConfBuf(entry[2]), lockOperPath+"/trigger");
                        owner.maapi.setElem(th, new ConfBuf(entry[3]), lockOperPath+"/unlock");
                    }
                }

                // Cache booleans in oper data
                if (booleans > 0) {
                    traceInfo(worker, "booleans: creating|updating "+booleans+" oper-data entries");
                    for (int i = 0; i < booleans; i++) {
                        final String[] entry = operBooleanSet.get(i);
                        final String operPath = entry[0];
                        final String path = entry[1];
                        final String cmd = entry[2];
                        final String showWhenSet = entry[3];
                        if (owner.maapi.exists(th, operPath)) {
                            traceDev(worker, "booleans: updated '"+cmd+"' show-when-set="+showWhenSet);
                        } else {
                            traceDev(worker, "booleans: created '"+cmd+"' show-when-set="+showWhenSet);
                            owner.maapi.create(th, operPath);
                            owner.maapi.setElem(th, new ConfBuf(path), operPath+"/path");
                            owner.maapi.setElem(th, new ConfBuf(showWhenSet), operPath+"/show-when-set");
                        }
                        owner.maapi.setElem(th, new ConfBuf(cmd), operPath+"/cmd");
                    }
                }

                // Done, commit new oper-data entries
                owner.maapi.applyTrans(th, false);
                owner.maapi.finishTrans(th);
            }
            ...
            //
            if ("from".equals(dir)) {
                boolean modifyElem = false;
                List<String> lines = new ArrayList<>();

                // ned-defaults-boolean-diff extension
                int th2 = owner.maapi.startTrans(Conf.DB_OPERATIONAL, Conf.MODE_READ_WRITE);
                int num = owner.maapi.getNumberOfInstances(th2, owner.booleanOperPath);
                if (num > 0) {
                    int cth = owner.maapi.startTrans(Conf.DB_RUNNING, Conf.MODE_READ);

                    // Read the boolean-diff oper data list in one chunk
                    traceInfo(worker, "booleans: reading "+num+" oper-data entries");
                    MaapiCursor cr = owner.maapi.newCursor(th2, owner.booleanOperPath);
                    List<ConfObject[]> list = owner.maapi.getObjects(cr, 4, num);
                    for (int n = 0; n < list.size(); n++) {
                        final ConfObject[] objs = list.get(n);
                        final String id = objs[0].toString();
                        final String path = objs[1].toString();
                        final String cmd = objs[2].toString();
                        String showWhenSet = objs[3].toString();
                        final String devCmd = readBoolean.get(id);

                        // Auto-detect device behaviour, if it shows this leaf or not when set
                        if ("unknown".equals(showWhenSet)) {
                            if (devCmd != null) {
                                showWhenSet = devCmd.trim().startsWith("no ") ? "false" : "true";
                            } else {
                                showWhenSet = cmd.trim().startsWith("no ") ? "true" : "false";
                            }
                            traceDev(worker, "booleans: found show-when-set="+showWhenSet+" device");
                            final String operPath = String.format(owner.booleanOperPath + "{%s}", id);
                            owner.maapi.setElem(th2, new ConfBuf(showWhenSet), operPath+"/show-when-set");
                            modifyElem = true;
                        }

                        // Auto-inject hidden default to avoid diff
                        final String value = owner.maapiGetLeafString(worker, cth, path);
                        final String isAdd = cmd.trim().startsWith("no ") ? "false" : "true";
                        traceDev(worker, "booleans: "+path+" CDB="+value+" cmd="+cmd+" show-when-set="+showWhenSet);
                        if (devCmd == null && value != null && !isAdd.equals(showWhenSet)) {
                            traceDev(worker, "booleans: injecting '"+cmd+"' on show-when-set="+showWhenSet+" device");
                            lines.add(cmd);
                        }
                    }

                    cr.destroy();
                    owner.maapi.finishTrans(cth);
                }

                // Finish transaction and commit updated oper-data entries
                if (modifyElem) {
                    owner.maapi.applyTrans(th2, false);
                }
                owner.maapi.finishTrans(th2);

                // Inject hidden boolean defaults in running-config when reading from device
                if (lines.size() > 0) {
                    traceInfo(worker, "booleans: injecting "+lines.size()+" hidden boolean default line(s)");
                    Schema.ParserContext extraCtx =
                        owner.newParserContext(worker, lines, Schema.ParserDirection.FROM_DEVICE, pctx.dataTree);
                    owner.disableAllExtensions();
                    extraCtx.parse();
                    owner.enableAllExtensions();
                }
            }

        } catch (Exception e) {
            owner.logError(worker, "ioscliextensions.done() exception error :: "+e.getMessage(), e);
        }

        traceInfo(worker, "done extension exit handler ("+dir+"-device) "+owner.tickToString(start));
    }
    ...
    /**
     * redeploy-with-change - Remove top-list and redeploy when list modified (unless only adds)
    ...
    public void redeployWithChange(final NedWorker worker, Schema.CallbackMetaData meta,
        ...
        // Check if entire list is created
        if (!owner.maapi.exists(fromT, path)) {
            traceDev(worker, "   ignored, list created");
            return;
        }
        ...
        String base = getMatch(cmd, meta.argument) + " ";
        traceDev(worker, "   regex = '"+meta.argument+"'  base = '"+base+"'");
        ...
        for (;pctx.peekNextLine() != null;) {
            ...
            next = pctx.popNextLine();
            lines.add(next);
        }

        // Only 1 line add|delete, ignore extension
        if (lines.size() == 1 && !cmd.startsWith("no ")) {
            traceDev(worker, "   ignored, single line add");
            return;
        }

        // Check if we should redeploy the list
        ...
        if (!haveDelete) {
            traceDev(worker, "   ignored, no deletes");
    ...
    /**
     * redeploy-with-change.mode - Remove top-list and redeploy when list modified
     * cli:direction "to-device"
     * cli:state "post-match"
     */
    public void redeployWithChangeMode(final NedWorker worker, Schema.CallbackMetaData meta,
                                   Schema.ParserContext pctx,
                                   final int fromT, final int toT) throws Exception {
        final String cmd = pctx.getCurrentLine().trim();
        traceCmd(worker, cmd);
        ...
        if (!owner.maapi.exists(toT, path)) {
            traceDev(worker, "   ignored, top-list deleted");
            return;
        }
    ...
    public void configLock(final NedWorker worker, Schema.CallbackMetaData meta,
        ...
        if (this.devDisableConfigLocks) {
            traceDev(worker, "   ignored, all config locks disabled by developer ned-setting");
            return;
        }

        final String arg = meta.argument;
        if (this.autoDisableConfigLockIfSp && arg.startsWith(" service-policy ")) {
            traceDev(worker, "   ignored, service-policy config locks disabled by auto ned-setting");
            return;
        }
        ...
        if (meta.extArguments != null) {
            if (meta.extArguments.contains(" :: ")) {
                ...
                prefix = meta.extArguments;
            }
        }
        traceDev(worker, "   trigger = "+stringQuote(trigger));

        // Create unlock config
        unlock += (spaces(cmd) + prefix + "no "+cmd.trim()+"\n");
        unlock += toExit(pctx);
        traceDev(worker, "   unlock = "+stringQuote(unlock));

        // Cache lock entry to add when finished
        String[] entry = new String[4];
        entry[0] = lockOperPath;
        entry[1] = path;
        entry[2] = trigger;
        entry[3] = unlock;
        operLockSet.add(entry);

        traceInfo(worker, "locks: cached lock update for "+shortpath(path));
    }
     ...
     * cli:state "post-match"
     */
    public void replaceNoLeaf(NedWorker worker, Schema.CallbackMetaData meta, Schema.ParserContext pctx) {
        if (owner.isNetsim()) {
            return;
        }
        ...
        String cmd2 = cmd.replace("no-", "");
        if (cmd2.trim().startsWith("no ")) {
            cmd2 = cmd2.replace("no ", "");
        } else {
            cmd2 = spaces(cmd) + "no " + cmd2.trim();
        }

        traceOut(worker, "transformed '"+cmd+"' to '"+cmd2+"'");
        pctx.replaceCurrentLine(cmd2);
    }
    ...
    public void deleteSyntax(NedWorker worker, Schema.CallbackMetaData meta, Schema.ParserContext pctx) {
        ...
        if (cmd.contains(" ; ")) {
            List<String> inject = new ArrayList<>(Arrays.asList(cmd.split(" ; ")));
     ...
     * cli:state "post-match"
     */
    public void duplicateCreate(NedWorker worker, Schema.CallbackMetaData meta, Schema.ParserContext pctx) {
        if (owner.isNetsim()) {
            return;
        }
        ...
        traceOut(worker, "injected "+stringQuote(line));
    ...
    /**
     * default-when-created - inject default statement when this is created
     * cli:direction "to-device"
     * cli:state "post-match|call-once"
     */
    public void defaultWhenCreated(final NedWorker worker, Schema.CallbackMetaData meta,
                                  Schema.ParserContext pctx, final int fromT, final int toT) throws Exception {
        String cmd = pctx.getCurrentLine();
        if (owner.isNetsim() || cmd.trim().startsWith("no ")) {
            return;
        }
        final String path = pctx.getNCSCurrentKP(owner.device_id);
        if (owner.maapi.exists(fromT, path)) {
        ...
        if (visited.contains("default-when-created+"+path)) {
            return;
        }
        visited.add("default-when-created+"+path);

        final String[] args = meta.argument.split(" :: ");
        for (String d : args) {
            cmd = " default " + d;
            traceOut(worker, "clearing '"+cmd+"'");
            pctx.injectAfter(cmd);
        }
    }
    ...
    /**
     * password-quote-input
     * @param <regex>, where <STRING> is the string to quote
     * cli:direction "from-device"
     * cli:state "pre-match|call-once"
     */
    public void passwordQuoteInput(NedWorker worker, Schema.CallbackMetaData meta, Schema.ParserContext pctx) {
        if (owner.isNetsim()) {
            return;
        }

        String line = pctx.getCurrentLine();
        Pattern p = Pattern.compile(meta.argument.replace("<STRING>", "(\\S+)").replace(" ","[ ]+"));
        Matcher m = p.matcher(line);
        if (m.find()) {
            String text = m.group(1);
            traceIn(worker, "quoted '"+text+"' in "+line);
            pctx.injectImmediate(line.replace(text, passwordQuote(text)));
            pctx.skipCurrentLine();
        }
    }
    ...
    /**
     * transform-disabled-no
     * cli:direction "to-device";
     * cli:state "post-match"
     */
    public void transformDisabledNo(NedWorker worker, Schema.CallbackMetaData meta, Schema.ParserContext pctx) {
        ...
        String output;
        if (cmd.trim().startsWith("no ")) {
            output = meta.argument;
        } else {
            output = "no " + meta.argument;
        }
        traceOut(worker, "'"+cmd+"' to '"+output+"'");
        pctx.replaceCurrentLine(output);
    }
     ...
     * cli:state "post-match"
     */
    public void patternString(NedWorker worker, Schema.CallbackMetaData meta, Schema.ParserContext pctx) {
        if (owner.isNetsim()) {
            return;
        }
        final String cmd = pctx.getCurrentLine();
        ...
        // String val = pctx.getLeaf(".").getValue();
        String val = pctx.currentDataContext.value(null);
        traceDev(worker, "val = "+val);
    }
    ...
    /**
     * track-object-ignore-delete - ignore track object deletion when track type is changed
     * cli:direction "to-device"
     * cli:state "post-match"
     */
    public void trackObjectIgnoreDelete(final NedWorker worker, Schema.CallbackMetaData meta,
                                        Schema.ParserContext pctx, final int fromT, final int toT) {
        ...
        final String cmd = pctx.getCurrentLine();
        final String from = owner.maapiGetConfig(worker, fromT, parentPath, 0);
        final String to = owner.maapiGetConfig(worker, toT, parentPath, 0);
        if ((from != null) && (to != null)) {
            final String fromType = getMatch(from, "track \\d+ (\\S+)");
            final String toType = getMatch(to, "track \\d+ (\\S+)");
            if (!fromType.equals(toType)) {
                traceOut(worker, "ignored command '"+cmd+"'");
                pctx.skipCurrentLine();
            }
        }
    }
    ...
    /**
     * unlock-rewrite - temporarily remove 'rewrite ingress' line when modifying service encapsulation dot1q
     * cli:direction "to-device"
     * cli:state "post-match"
     */
    public void unlockRewrite(final NedWorker worker, Schema.CallbackMetaData meta,
                              Schema.ParserContext pctx,
                              final int fromT, final int toT) throws Exception {
        ...
        if (pctx.getState() != Schema.ParserState.MULTI_LINE) {
            ...
            final String ncsPath = ncspath(path);
            if (!owner.maapi.exists(fromT, ncsPath)) {
                return; // encapsulation is created in this transaction
            }

            final String target = ncsPath + "/../../rewrite/ingress/tag/pop";
            if (!maapiExists(worker, toT, target)) {
                return; // rewrite ingress tag pop not set (or deleted)
            }
            if (!maapiExists(worker, fromT, target)) {
                return; // rewrite ingress set in this transaction
            }

            // A list change, collect entire list in order to temporarily disable rewrite
            pctx.startMultiLine(meta);
            pctx.muteCallback(path, meta);
            return;
        }

        // Collect entire service instance changes before modifying the block
        ...
        //
        // Temporarily disable rewrite ingress
        //
        traceOut(worker, "temporarily disable rewrite ingress in "+shortpath(path));

        final String target = path + "/../../rewrite";
        final String to = owner.maapiGetConfig(worker, toT, target, 2);
        traceOut(worker, "redeployed: "+stringQuote(to)+" in interface service instance");

        // (1) Add delete of rewrite ingress
        List<String> inject = new ArrayList<>();
        inject.add("  no "+to.trim());

        // (2) Add transaction changes (excluding exit)
        for (int n = 0; n < lines.size() - 1; n++) {
            inject.add(lines.get(n));
        }

        // (3) Add back rewrite ingress
        inject.add("  "+to.trim());

        // (4) Finally add exit line
        inject.add(lines.get(lines.size() -1));

        // Inject modified block
        pctx.injectImmediate(inject);
    }
    ...
    public void ifVrfRestore(final NedWorker worker, Schema.CallbackMetaData meta,
        ...
        for (String line : lines) {
            ...
            if (line.matches("^ (?:no )?vrrp \\d+ ip \\S+$")) {
                continue; // ' vrrp * ip <a.b.c.d>'  NOTE: no-line is missed due to dependency
            }
            if (!v4only) {
    ...
    private int maapiGetIfAddrs(NedWorker worker, int th, String ifpath, boolean v4only, List<String> inject)
        ...
        try {
            ...
            //
            // (2) interface * / vrrp * ip <a.b.c.d>
            //
            final String vPath = ifpath+"/vrrp";
            if (maapiExistsNoException(worker, th, vPath)) {
                final int num = owner.maapi.getNumberOfInstances(th, vPath);
                if (num > 0) {
                    final MaapiCursor cr = owner.maapi.newCursor(th, vPath);
                    List<ConfObject[]> vrrpList = owner.maapi.getObjects(cr, 1, num);
                    for (int n = 0; n < vrrpList.size(); n++) {
                        ConfObject[] objs = vrrpList.get(n);
                        final String vId = objs[0].toString();
                        String ip = owner.maapiGetLeafString(worker, th, vPath+"{"+vId+"}/ip/address");
                        if (ip != null) {
                            String vLine = " vrrp "+vId+" ip "+ip;
                            traceOut(worker, "redeployed '"+vLine+"'");
                            sb.append(vLine+"\n");
                            added++;
                        }
                    }
    ...
    public void trimDeleteWhenEmpty(final NedWorker worker, Schema.CallbackMetaData meta,
        ...
        final String key = "trim-delete-when-empty+"+path+"+"+transformed;
        if (visited.contains(key)) {
            traceOut(worker, "stripped delete '"+cmd+"'");
            pctx.skipCurrentLine();
        } else {
    ...
    /**
     * trim-when-list - strip create|delete line if parent list deleted
     * cli:direction "to-device"
     * cli:state "post-match"
     */
     ...
     * @throws Exception
     */
    public void trimBadNsoDelete(final NedWorker worker, Schema.CallbackMetaData meta,
                                 Schema.ParserContext pctx, final int fromT, final int toT) {
        final String cmd = pctx.getCurrentLine();
        traceCmd(worker, cmd);
        if (!cmd.trim().startsWith("no ")) {
        ...
        traceOut(worker, "trimmed bad delete "+stringQuote(cmd)+" [nso-patch]");
        pctx.skipCurrentLine();
    }
    ...
    public void bgpNbrRedeployTrigger(final NedWorker worker, Schema.CallbackMetaData meta,
        ...
        final String nbr = m.group(1);

        if (visited.contains("bgp-nbr-redeploy-trigger-"+nbr)) {
            traceDev(worker, "   ignored, visited");
            return;
        }
        ...
        traceVerbose(worker, "   bgp: triggered neighbor redeploy for '"+nbr+"'");
        owner.extBgpNbrRedeploy.put(nbr, path);

        visited.add("bgp-nbr-redeploy-trigger-"+nbr);
        pctx.muteCallback(pctx.getCurrentKeyPath(), meta);
    }


    /* delete-table-map-auto-config
     * cli:direction "to-device"
     * cli:state "post-match"
     */
    public void deleteTableMapAutoConfig(final NedWorker worker, Schema.CallbackMetaData meta,
                               Schema.ParserContext pctx) {
        if (!pctx.isDelete() || owner.isNetsim()) {
            return;
        }
        final String[] injectLines = {
            "no table-map policed-dscp",
            "no table-map AutoQos-4.0-Trust-Cos-Table",
            "no table-map AutoQos-4.0-Trust-Dscp-Table"
        };

        for (String line : injectLines) {
            pctx.injectAfter(line);
            traceOut(worker, "post-injected "+line);
        }
    }


    /*
     * remove-before-change-leaf
     * cli:direction "to-device"
     * cli:state "post-match"
     */
    public void removeBeforeChangeLeaf(final NedWorker worker, Schema.CallbackMetaData meta,
                                       Schema.ParserContext pctx,
                                       final int fromT, final int toT) {
        String cmd = pctx.getCurrentLine();
        traceCmd(worker, cmd);
        if (cmd.trim().startsWith("no ")) {
            visited.add("remove-before-change-leaf-"+cmd.trim());
            return;
        }

        final String path = pctx.getNCSCurrentKP(owner.device_id);
        if (!maapiExists(worker, fromT, path)) {
            return; // top list created
        }
        if (!maapiExists(worker, toT, path)) {
            return; // top list deleted
        }

        final String leafPath = path + "/" + meta.argument;
        if (maapiExists(worker, toT, leafPath)) {
            return; // leaf exists, i.e. not deleted in this trans
        }
        if (!maapiExists(worker, fromT, leafPath)) {
            return; // leaf was not set, i.e. not deleted in this trans
        }

        String entry = "no " + owner.maapiGetConfig(worker, fromT, path, 0);
        if (visited.contains("remove-before-change-leaf-"+entry.trim())) {
            traceDev(worker, "   ignored, delete already exists");
            return;
        }

        traceOut(worker, "pre-injected "+stringQuote(entry));
        pctx.injectBefore(entry);

        visited.add("remove-before-change-leaf-"+entry);
        pctx.muteCallback(pctx.getCurrentKeyPath(), meta);
    }


    /*
     * re-enter-parent-mode
     * cli:direction "to-device"
     * cli:state "post-match"
     *
     * // key chain * / key *
     * device exits 'key chain' mode when last entry of key deleted.
     */
    public void reEnterParentMode(final NedWorker worker, Schema.CallbackMetaData meta, Schema.ParserContext pctx,
                                  final int fromT, final int toT) throws Exception {
        if (!pctx.isDelete() || owner.isNetsim()) {
            return;
        }

        if (!pctx.peekNextLine().trim().startsWith("no key ")) {
            final String path = pctx.getNCSCurrentKP(owner.device_id)
                    .substring(0, pctx.getNCSCurrentKP(owner.device_id).lastIndexOf("{"));
            if (maapiExistsNoException(worker, fromT, path)) {
                final int num = owner.maapi.getNumberOfInstances(fromT, path);
                if (num > 0) {
                    final MaapiCursor cr = owner.maapi.newCursor(fromT, path);
                    List<ConfObject[]> keyList = owner.maapi.getObjects(cr, 1, num);
                    for (int n = 0; n < keyList.size(); n++) {
                        ConfObject[] objs = keyList.get(n);
                        final String key = objs[0].toString();
                        if (maapiExistsNoException(worker, toT, path + "{" + key + "}")) {
                            // no full list delete, no need to re-enter mode.
                            return;
                        }
                    }
                    final String mode = pctx.getEnterContextLines().get(0).trim();
                    traceVerbose(worker,
                            "injecting mode " + stringQuote(mode) + " after " + stringQuote(pctx.getCurrentLine()));
                    pctx.injectAfter(pctx.currentIndent() + mode);
                    cr.destroy();
                }
            }
        }
    ...
    /**
     * ned-defaults-boolean-diff
     * cli:direction "both";
     * cli:state "post-match";
     */
    public void booleanDiff(NedWorker worker, Schema.CallbackMetaData meta, Schema.ParserContext pctx)
        throws Exception {

        if (owner.isDry || owner.isNetsim()) {
            return;
        }
        ...
        // Create unique list id and derive oper path
        final String path = pctx.getNCSCurrentKP(owner.device_id);
        final String id = calculateMd5Sum(path);
        final String operPath = String.format(owner.booleanOperPath + "{%s}", id);
        ConfPath cp = new ConfPath(operPath);

        // oper-data entry template
        String[] entry = new String[4];
        entry[0] = operPath;
        entry[1] = path;
        entry[2] = cmd;
        entry[3] = "?";  // show-when-set
        ...
        if (pctx.parserDirection == Schema.ParserDirection.TO_DEVICE) {
            ...
            if (!owner.cdbOper.exists(cp)) {
                traceDev(worker, "booleans: setting '"+cmd+"' on device ["+id+"]");
                entry[3] = "unknown";
            } else {
                traceDev(worker, "booleans: modifying '"+cmd+"' on device ["+id+"]");
            }
            operBooleanSet.add(entry);
        }
        ...
        //
            ...
            readBoolean.put(id, cmd);
            if (owner.cdbOper.exists(cp)) {
                traceDev(worker, "booleans: confirmed '"+cmd+"' on device ["+id+"]");
            } else {
                traceDev(worker, "booleans: detected '"+cmd+"' on device ["+id+"]");
                entry[3] = cmd.trim().startsWith("no ") ? "false" : "true";
                operBooleanSet.add(entry);
            }
        }
    }
    ...
    /**
     * maapiExistsNoException
     * @param
     * @return
     */
    private boolean maapiExistsNoException(NedWorker worker, int th, String path) {
        try {
