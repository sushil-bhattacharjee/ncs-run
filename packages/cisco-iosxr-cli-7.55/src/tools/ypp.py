...
def version_wrapper(v):
    return LooseVersion(str(v))

class Static(object):
    variables = {"version": version_wrapper}
    verbose = False
...
def ypp(arguments):
    ...
    parser.add_argument("--removed-line")
        ...
        for name in gfiles:
            ...
            for line in text.split("\n"):
                ...
                if show or stack.show():
                    ...
                    vprint("HIDE '%s'" % line)
                    # to skip removed lines entirely use --removed-line=None
                    if args.removed_line != "None":
                        # Default to yang comment, if not allready commented
                        if args.removed_line == None:
                            if not line.strip().startswith("//"):
                                line = "// " + line
                        elif "%s" in args.removed_line:
                            line = args.removed_line % line
                        else:
                            line = args.removed_line
                        ntext.append(line)
