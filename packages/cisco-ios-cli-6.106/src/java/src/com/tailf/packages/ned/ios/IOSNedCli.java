...
import com.tailf.packages.ned.nedcom.Schemas;
import com.tailf.packages.ned.nedcom.NedComCliBase;
import com.tailf.packages.ned.nedcom.NedInterface;
...
import com.tailf.packages.ned.nedcom.connector.CliConnectorNedUtils;
...
public class IOSNedCli extends NedComCliBase {
    ...
    // NOTE: must match tailf-ned-cisco-ios-oper.yang locks 'cisco-ios-oper locks' list
    protected static final int NUM_LOCKS_LEAVES = 4;
    ...
    private static final String[] staticWarning = {
        ...
        //??? " is disabled",
        "dynamic punt is disabled", // no cable dynamic-punt enable
        "snooping is disabled on this vlan", // ip igmp snooping vlan 32 querier
        ...
        "use of 'no' config will change to default level", // no license boot level
        ...
        "cws-tunnel .+ are created", // interface * / cws-tunnel
        ...
        "shutdown not allowed", // interface CEM* / no shutdown
        "bringing \\S+ the port due to config change", // interface * / no service instance *
        "no site-manager path configured for interface ", // no site-manager path
        ...
        "lines may not train with this rate if co is configured for enhanced rate", // controller SHDSL * / dsl-group * / shdsl rate
        ...
        "removing wide metrics also removed ipv6 multi-topology on", // router isis * / no metric-style wide
        ...
        "ldp remains enabled on interface ", // interface * / no mpls ip
        ...
        "acl attached does not exist, please attach correct acl", // ip http access-class ipv4
        ...
        "modulation profile set to", // no us-channel 12 subcarrier-spacing 50KHz
        ...
        "replication type not configured, configuration incomplete", // l2vpn evpn instance * vlan-based
        "incomplete evpn instance config", // l2vpn evpn instance * vlan-based / *
        ...
        "cloud ha bfd not enabled until configured on peer", // redundancy / cloud * / bfd peer
        ...
        "ena/dis svi on ucse needs a oir or router reload", // no ucse subslot *
        "dspfarm profile \\d+ :: ", // dspfarm profile * transcode universal
        "ucse[_]config[_]commands ", // ucse subslot * / imc ip address
        "error deleting flash", // no voice register global
        "reset the gateway on ccm if this gateway", // mgcp fax t38 inhibit
        "the mgcp process is currently active", // mgcp

        // mobile
        "de[-]register with mcsa done", // no ipv6 mobile pmipv6-mag
    ...
    private static final String[] staticNoWarning = {
        ...
        "dn tag too large, maximum", // no voice register dn *
        "certificate server .+ is not known" // no crypto pki server
    };
    ...
    protected String booleanOperPath;
    ...
    private boolean haveShowBootVar = true;
    ...
    private boolean haveShowIfSp = true;
    private String showIfSpDump = "";

    // NED-SETTINGS
    protected boolean newAAAList;
    protected boolean cmMatchMultiLine;
    ...
    private String readTransIdMethod;
    ...
    public IOSNedCli(String deviceId,
        ...
        this.booleanOperPath = operRoot + "/defaults-boolean-diff";
    }
     ...
     */
    @Override
    public NedLogger createLogger(NedInterface ned) {
        return super.createLogger(this);
    }

    protected boolean logIsVerbose() {
        return (log.getLevel() == NedLogger.Level.VERBOSE || log.getLevel() == NedLogger.Level.DEBUG);
    }
    protected boolean logIsDebug() {
        return log.getLevel() == NedLogger.Level.DEBUG;
    }

    public void logError(NedWorker worker, String msg) {
        log.error(msg);
    }
    public void logError(NedWorker worker, String msg, Exception e) {
        log.error(e, msg);
    }
    public void logVerbose(NedWorker worker, String msg) {
        log.verbose(msg);
    }

    public void traceInfo(NedWorker worker, String msg) {
        log.getLogger(NedLogger.Type.TRACE).info(msg);
    }
    public void traceVerbose(NedWorker worker, String msg) {
        log.getLogger(NedLogger.Type.TRACE).verbose(msg);
    }
    public void traceDev(NedWorker worker, String msg) {
        log.getLogger(NedLogger.Type.TRACE).debug(msg);
    }
    ...
    protected void setupParserContext(Schema.ParserContext parserContext) {
        ...
        if (parserContext.parserDirection == Schema.ParserDirection.FROM_DEVICE) {
            ...
            if (cmMatchMultiLine) {
                traceVerbose(worker, "adding /tailfned/api/class-map-match-multi-line-format");
                parserContext.addVirtualLeaf("/tailfned/api/class-map-match-multi-line-format", "");
            }
        }
    }
    ...
    public void nedSettingsDidChange(NedWorker worker, Set<String> changedKeys, boolean isConnected) throws Exception {
        ...
        try {
            ...
            // read
            //
            readTransIdMethod = nedSettings.getString("read/transaction-id-method");
            ...
            cmMatchMultiLine = nedSettings.getBoolean("api/class-map-match-multi-line-format");
    ...
    protected PlatformInfo setupDevice(NedWorker worker) throws Exception {
        ...
        try {
            ...
            if (version.contains("NETSIM")) {
                ...
                // Override SCP or other bad global setting
            ...
            // REAL DEVICE
                ...
                if ("config-id".equals(readTransIdMethod)) {
    ...
    public void connectDevice(NedWorker worker) throws Exception {
        ...
        CliConnectorNedUtils.connectorConnectDevice(this, worker);
    ...
    protected void setupInstance(NedWorker worker, PlatformInfo platformInfo) throws Exception {
        ...
        String[][] defaultAutoPrompts = new String[][] {
            ...
            { " profile MUST have:", null }, // crypto ikev2 profile *
            { ":\\s*$", "<prompt>" },
            { "\\][\\?]?\\s*$", "<prompt>" }
        };
    ...
    private void setupNedDiff(NedWorker worker) {
        ...
        nedDiff = new NedDiff(this, logIsDebug());
        ...
        nedDiff.add(bgpMode, "^neighbor <STAY> peer-group \\S+ :: after :: >neighbor <STAY> remote-as ");
        ...
        ignore.append(":>default "); // Ignore default statement injected by default-when-created extension
    ...
    private class ShowHandler extends NedLiveStatsShowHandler {
        public ShowHandler(NedConnectionBase owner, CliSession session, Pattern[] prompts)
     ...
     */
    @Override
    public String getPlatformData(int thr, String leaf) throws Exception {
        // First try devices device platform
        String value = super.getPlatformData(thr, leaf);
        if (UNKNOWN.equals(value) && includeCachedShowVersion) {
            // Second try config cached-show version
            String p = confRoot + "cached-show/version/" + leaf;
            try {
                if (maapi.exists(thr, p)) {
                    value = ConfValue.getStringByValue(p, maapi.getElem(thr, p));
                }
            } catch (MaapiException ignore) {
                // Ignore Exception
            }
    ...
    private String getIosPolice(NedWorker worker, int thr, boolean cdbLookupOk) {
        ...
        } else if (iosmodel.contains("WS-C45")) {
            police = "cirmode-bpsflat"; // WS-C4510R-E
    ...
    /**
     * Retrieve running config from device (sync-from or compare-config)
     * @param
     * @throws Exception
     */
    @Override
    public void show(NedWorker worker, String toptag) throws Exception {
        ...
        // Reset locks before calling all turbo extensions
        locks.reset(worker);

        // Upload device config to NSO using turbo-parser
        parseAndLoadXMLConfigStream(maapi, worker, schema, res);

        // New NSO feature, set provisional transaction-id in show()
        if (readTransIdMethod.startsWith("config-hash")) {
    ...
    private String showRunningConfig(NedWorker worker) throws Exception {
        ...
        if ("show".equals(failphase)) {
            traceInfo(worker, "debug: simulating show never returning");
            sleep(worker, 10000000, true);
        }
    ...
    private String getConfigOnly(NedWorker worker, String res) throws Exception {
        ...
        try {
            ...
            if (cmMatchMultiLine) {
                traceInfo(worker, "transformed <= inserted tailfned api class-map-match-multi-line-format");
                first.append("tailfned api class-map-match-multi-line-format\n");
            }
            ...
            if (isDevice()) {
                ...
                // IOS changed the hidden default of 'service pad' in IOS version 17.3 or newer
                final String minver = getMatch(iosversion, "17[.](?:0)?(\\d+).*");
                if (minver != null && Integer.parseInt(minver) >= 3 && !res.contains("\nservice pad")) {
                    traceInfo(worker, "transformed <= inserted 'no service pad' [IOS 17."+minver+" patch]");
    ...
    private String getExtraConfig(NedWorker worker, String res) throws Exception {
        ...
        // Cache interfaces switchport status
        this.showIfSpDump = "";
        if (autoInterfaceSwitchportStatus && haveShowIfSp) {
            traceInfo(worker, "extracting config - show interfaces switchport");
            this.showIfSpDump = print_line_exec(worker, "show interfaces switchport");
            if (this.showIfSpDump.trim().isEmpty()) {
                // Note: If switchport is not enabled on a single interface then 'show interfaces switchport' returns nothing
                traceInfo(worker, "'show interfaces switchport' returned nothing, switchport is not enabled on any interface");
            }
            else if (isExecError(this.showIfSpDump)) {
                traceInfo(worker, "notice: missing support for 'show interfaces switchport'");
                haveShowIfSp = false;
            }
        }
    ...
    private void getConfigVersion(NedWorker worker, StringBuilder sb) throws Exception {
        ...
        traceInfo(worker, "extracting config - show version");
        String showbuf = print_line_exec(worker, "show version");

        // service password-recovery
        if (showbuf.contains(" password-recovery mechanism is enabled")) {
            traceInfo(worker, "transformed <= inserted 'service password-recovery' from 'show version'");
            sb.append("service password-recovery\n");
        } else if (showbuf.contains(" password-recovery mechanism is disabled")) {
        ...
        // First try 'show bootvar' to get config-register
        String bootbuf = "";
        if (haveShowBootVar) {
            traceInfo(worker, "extracting config - show bootvar");
            bootbuf = print_line_exec(worker, "show bootvar");
            if (isExecError(bootbuf)) {
                haveShowBootVar = false;
            }
        }

        // config-register
        String where = "'show bootvar'";
        String match = getMatch(bootbuf, "\nConfiguration register is (\\S+)");
        if (match == null) {
            where = "'show version'";
            match = getMatch(showbuf, "\nConfiguration register is (\\S+)");
        }
        if (match != null) {
            traceInfo(worker, "transformed <= inserted 'config-register "+match+"' from " + where);
            sb.append("config-register "+match+"\n");
        }
    }
    ...
    private String scpGetConfig(NedWorker worker) throws Exception {
        ...
        try {
            ...
            in = new SCPInputStream(scpClient, scpSession);
            ...
            if (in != null) {
                try {
                    ...
                    // ignore for servers which close connection first
                }
            }
            if (scpSession != null) {
                scpSession.close();
    ...
    private String trimInput(NedWorker worker, String res) {
        ...
        for (int n = 0; n < lines.length; n++) {
            ...
            // Strip incomplete comments (e.g. crypto ikev2 profile)
                ...
                strip = true;
            } else if (trimmed.startsWith("! Incomplete EVPN instance config")) {
                strip = true;
            } else if (trimmed.startsWith("! This profile is incomplete")) {
                strip = true;
            } else if (trimmed.endsWith("! Incomplete or invalid interface")) {
                traceDev(worker, "transformed <= trimmed comment from '"+line+"'");
                line = line.replace("! Incomplete or invalid interface", "");
            }
            ...
            if (strip) {
                traceDev(worker, "transformed <= stripped '"+line+"'");
            } else {
                sb.append(line+"\n");
            }
        }
    ...
    private String modifyInput(NedWorker worker, String res) throws Exception {
        ...
        //
        traceInfo(worker, "quoting multi-line texts begin");
        long start1 = tick(0);
        String[] quoteTexts = {
            // menu <name> title ^C
            // <title text>
        ...
        for (int n = 0; n < quoteTexts.length; n++) {
            ...
            while (m.find()) {
                ...
                traceDev(worker, "transformed <= quoted '"+line+"' text");
                m.appendReplacement(sb, Matcher.quoteReplacement("\n"+line+" "+quoted));
            }
            m.appendTail(sb);
            res = sb.toString();
        }
        traceInfo(worker, "quoting multi-line texts done "+tickToString(start1));

        //
        // MAIN LINE-BY-LINE LOOP
        //
        start1 = tick(0);
        traceInfo(worker, "transforming config line-by-line begin");
        String toptag = "";
        int numTrapsDev = 0;
        String[] lines = res.split("\n");
        ...
        for (int n = 0; n < lines.length; n++) {
            ...
            //
            else if (toptag.startsWith("vlan configuration ") && line.startsWith("ip ")) {
                input = " " + line;
            }
            ...
            // controller SHDSL *
            //
            else if (toptag.startsWith("controller SHDSL") && line.startsWith(" dsl-group ")) {
                input = line.replace(", ", ",");
            }
            ...
            //
            // crypto ikev2 authorization policy * / no route accept
            //
            else if (toptag.startsWith("crypto ikev2 authorization policy ") && trimmed.equals("no route accept")) {
                input = line.replace("no route accept", "no route accept any");
            }
            ...
            //
            else if ("no crypto ikev2 proposal default".equals(trimmed)) {
                input = "crypto ikev2 proposal-default-disabled";
            }
            ...
            //
                ...
                for (int t = 1; t < tokens.length; t++) {
                    if ((match = getMatch(tokens[t], "\\S+ (?:drops|includes) (.+)")) != null) {
                        ...
                        tokens[t] = tokens[t].replace(match, stringQuote(replacement));
                    }
                    input += tokens[t];
                }
            }
            ...
            //
                ...
                if (input.contains(" transport udp") && !input.contains(" port ")) {
                    input = input.replace(" transport udp", " transport udp port 514");
                }
                if (input.contains(" transport tcp") && !input.contains(" port ")) {
                    input = input.replace(" transport tcp", " transport tcp port 601");
                }
            }
            ...
            //
                ...
                // policy-map * / class * / police - insert missing [cir|bc|be]
                    ...
                    traceVerbose(worker, "attempting auto-inject of cir|bc|be in "+toptag+" / class * / "+trimmed);
            ...
            } else if (line.startsWith("no ipv6 pim ") && line.contains(" rp embedded")) {
                input = line.replace("no ", "").replace("rp embedded", "rp no-embedded");
            }
        ...
        traceInfo(worker, "transforming config line-by-line done "+tickToString(start1));
        ...
        start1 = tick(0);
        traceInfo(worker, "transforming config line-by-line #2 begin");
        lines = res.split("\n");
        for (int n = 0; n < lines.length; n++) {
            String line = lines[n];
            String trimmed = line.trim();
            ...
            if (session != null && line.startsWith("interface ")) {
                ...
                // ned-settings cisco-ios auto interface-switchport-status true
                if (autoInterfaceSwitchportStatus && haveShowIfSp
                    && (line.contains("Ethernet") || line.contains("Port-channel") || line.contains("GigE"))) {
                    final String regex = "Name[:][ ]+"+shortenInterfaceName(line.replace("interface ", "").trim())+"\r\n(Switchport[:] \\S+)";
                    if ((res = getMatch(showIfSpDump, regex)) == null) {
                        res = print_line_exec(worker, "show " + trimmed + " switchport | i Switchport");
                    }
                    if (res.contains("Switchport: Enabled")) {
                        traceVerbose(worker, "transformed <= auto-injected 'switchport' in "+trimmed+"'");
                        sbin.append(" switchport\n");
                    } else if (res.contains("Switchport: Disabled")) {
                        traceVerbose(worker, "transformed <= auto-injected 'no switchport' in "+trimmed+"'");
                        sbin.append(" no switchport\n");
                    } else {
                        traceInfo(worker, "notice: failed to determine interface switchport status for "+stringQuote(trimmed));
                    }
                }
                continue;
            }
        ...
        traceInfo(worker, "transforming config line-by-line #2 done "+tickToString(start1));
        ...
        //
        start1 = tick(0);
        traceVerbose(worker, "transforming line config begin");
        res = sbin.toString();
        res = modifyInputExpandLine(worker, res, lineDump);
        if (expandedLineVtyFormat) {
            res = modifyInputExpandLineVty(worker, res);
        }
        traceVerbose(worker, "transforming line config done "+tickToString(start1));
     ...
     */
    private static final String[][] ifNameMap = {
        { "Et",  "Ethernet" },
        { "Fa",  "FastEthernet" },
        { "Gi",  "GigabitEthernet" },
     ...
     * @param
     * @return Short interface name
     */
    private String shortenInterfaceName(String name) {
        for (int i = 0; i < ifNameMap.length; i++) {
            if (name.startsWith(ifNameMap[i][1])) {
                return name.replace(ifNameMap[i][1], ifNameMap[i][0]);
            }
        }
        return name;
    }
    ...
    public void showOffline(NedWorker worker, String toptag, String data) throws Exception {
        try {
            ...
            this.logIgnoredDeletes = true;
            this.showOfflineIgnoreDelete = !nedSettings.getBoolean(NedSettings.LOAD_NATIVE_CFG_ALLOW_DELETE, false);
            this.showOfflineDeleteWithRemove = nedSettings.getBoolean(NedSettings.LOAD_NATIVE_CFG_DELETE_WITH_REMOVE, false);
            show(worker, toptag);
            traceInfo(worker, "done show-offline");
        } finally {
            this.logIgnoredDeletes = false;
            this.showOfflineIgnoreDelete = true;
            this.showOfflineDeleteWithRemove = false;
    ...
    public void getTransId(NedWorker worker) throws Exception {
        ...
        traceInfo(worker, "begin get-trans-id ("+readTransIdMethod+")");
        ...
        try {
            ...
            String res = null;
            String which = "GET";
            if (isNetsim() && "confd-state-trans-id".equals(readTransIdMethod)) {
            ...
            else if (readTransIdMethod.startsWith("config-hash") && this.lastTransactionId != null) {
                traceInfo(worker, "cached trans_id = "+this.lastTransactionId);
            }

            // (3) Use last cached transformed config from applyConfig() secret code
            else if (readTransIdMethod.startsWith("config-hash") && this.lastTransformedConfig != null) {
                res = this.lastTransformedConfig;
                which = "APPLY";
            }

            // (4) Use 'Last configuration change' string from running-config
            else if ("last-config-change".equals(readTransIdMethod) && isDevice()) {
            ...
            else if ("config-id".equals(readTransIdMethod) && isDevice()) {
            ...
            else if ("config-history".equals(readTransIdMethod) && isDevice()) {
            ...
            // (7) Get config from device and transform it
                ...
                traceVerbose(worker, "booleans: triggering ned-defaults-boolean-diff from-device extensions from get-trans-id");
                parseConfig(worker, Arrays.asList(res.split("\n")), Schema.ParserDirection.FROM_DEVICE, NedState.TRANS_ID, true,
                            Arrays.asList(new String[]{"ios:ned-defaults-boolean-diff"}));
            }
    ...
    private String calculateTransId(NedWorker worker, String res) throws Exception {
        ...
        if ("config-hash-modeled".equals(readTransIdMethod)) {
    ...
    public void prepareDry(NedWorker worker, String data) throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < lines.length; i++) {
                String line = lines[i];
                if (!logIsVerbose() && line.startsWith("!!")) {
                    continue; // discard line in dry output
                } else if (logIsVerbose() && line.startsWith("tailfned ")) {
                    sb.append(line+"\n");
                } else {
                    // Modify texts
     ...
     * @return
     */
    private String modifyOutputLineNetsim(String data) {
    ...
    private String modifyOutputLine(NedWorker worker, String data, int fromTh, int toTh) throws NedException {
        ...
        for (int n = 0; n < lines.length; n++) {
            ...
            //
            else if (getMatch(line, "^no logging source-interface (\\S+)$") != null
            ...
            // no crypto ikev2 proposal default
            //
            else if ("no crypto ikev2 proposal default".equals(line)) {
                output = "default crypto ikev2 proposal";
            }
            ...
            //
            else if ("crypto ikev2 proposal-default-disabled".equals(line)) {
                output = "no crypto ikev2 proposal default";
            } else if ("no crypto ikev2 proposal-default-disabled".equals(line)) {
                output = "default crypto ikev2 proposal";
            }

            /*
               else if ("no crypto ikev2 proposal default".equals(line)) {
                if (data.contains("\ncrypto ikev2 proposal-default-disabled\n")) {
                    continue;
                }
                sb.append(line+"\n");
                traceVerbose(worker, "transformed => injected 'default crypto ikev2 proposal'");
                output = "default crypto ikev2 proposal"; // remove no-entry on device
                }
            */
            ...
            //
                     ...
                     && trimmed.startsWith("neighbor ") && trimmed.contains(" password 7 ")
                     && (match = getMatch(line, "(\\s+neighbor \\S+ password )")) != null) {
                // IOS bug patch: router bgp * / neighbor * password 7 [CISCOIOS-1418]
                traceVerbose(worker, "transformed => pre-injected dummy password in "+match);
                sb.append(match + "0 bgp-password-patch\n");
            }

            //
            // router bgp * / template peer-session * / password
            //
            else if (autoBgpNbrPasswordPatch && toptag.startsWith("router bgp ") && line.startsWith("  password 7 ")) {
                traceVerbose(worker, "transformed => pre-injected dummy password in bgp peer-session template");
                sb.append("  password 0 bgp-password-patch\n");
            }
            ...
            //
            // voice register global / auto-register
            //
            else if ("voice register global".equals(toptag) && cmdtrim.equals("auto-register")) {
                output = " !"+trimmed;
            }
            ...
            //
                ...
                output = line.replace("ip access-list unordered ", "ip access-list ");
    ...
    private String modifyOutput(NedWorker worker, String data, String function) throws NedException {
        ...
        try {
            ...
            // Modify NSO output
            data = modifyNsoOutput(worker, data, toTh);
     ...
     * @throws NedException
     */
    private String modifyNsoOutput(NedWorker worker, String data, int toTh) throws NedException {

        try {
            Set<String> pmNameChecked = new HashSet<>();
            List<ConfObject[]> lockList = null;

            //
            // (1) SECRETS pre-reordering due to NSO bug
            //
            NedDiff preDiff = new NedDiff(this, logIsDebug());
		    ...
		    // fall through to add interface line
                ...
                // class-map *
                if (isDevice() && line.startsWith("class-map")
                    && !nedSettings.getBoolean("developer/disable-config-locks", false)) {

                    // Read locks list from oper data in CDB
                    if (lockList == null) {
                        final String operPath = operRoot + "/locks";
                        final int num = cdbOper.getNumberOfInstances(operPath);
                        if (num > 0) {
                            traceVerbose(worker, "locks: read "+num+" config lock(s)");
                            lockList = cdbOper.getObjects(NUM_LOCKS_LEAVES, 0, num, operPath);
                        }
                    }

                    // Scan policy-map locks
                    if (lockList != null) {
                        for (int l = 0; l < lockList.size(); l++) {
                            final ConfObject[] objs = lockList.get(l);

                            final String trigger = objs[2].toString();
                            if (!trigger.startsWith("policy-map ")) {
                                continue;
                            }

                            final String pmName = getMatch(trigger, "policy-map (\\S+)");
                            if (pmNameChecked.contains(pmName)) {
                                continue;
                            }
                            pmNameChecked.add(pmName);

                            traceVerbose(worker, "locks: checking "+trigger);
                            final String pmConfig = maapiGetConfig(worker, toTh, confRoot+"policy-map{"+pmName+"}/", 0);
                            if (pmConfig != null) {
                                final String cmName = getMatch(line, "class-map \\S+ (\\S+)");
                                if (pmConfig.contains("\n class "+cmName+"\n")) {
                                    traceVerbose(worker, "transformed => injecting dummy '"+trigger+"' reference to trigger unlocking mechanism");
                                    sb.append(trigger+"\nexit\n");
                                }
                            }
                        }
            ...
            throw new NedException("internal error in modifyNsoOutput: " + e.getMessage(), e);
        }
    }


    /**
     * Act on info cached by 'bgp-nbr-redeploy-trigger' to redeploy neighbor entries when peer-group|remote-as is changed
     * @param
     * @return
     */
            ...
            for (; n < lines.length; n++) {
                ...
                if (nbr == null) {
                    ...
                    sb.append(line+"\n");
                           ...
                           (line.startsWith(" "+nbr+"peer-group ") || line.startsWith(" "+nbr+"remote-as "))) {
    ...
    private String trimInterfaces(NedWorker worker, String data, int fromTh) {
        ...
        for (int n = 0; n < lines.length; n++) {
            ...
            // Trim NSO bug with interface * / xconnect / no mpls control-word'
            else if (   "  xconnect".equals(line)
                        && "   no mpls control-word".equals(nextline)
                        && "  exit".equals(nextline2)) {
                traceInfo(worker, "transformed => stripped invalid xconnect / no mpls control-word lines [nso-patch]");
                n = n + 2; // Skip the entire bad xconnect block
            }
        ...
        for (int n = 0; n < lines.length; n = next) {
            ...
            for (;next < lines.length;) {
                ...
                sm.put(Integer.valueOf(end), lines[next]);
                next = nextnext;
            }
    ...
    private String trimOutput(NedWorker worker, String data, int toTh) {
        ...
        for (int n = 0; n < lines.length; n++) {
            ...
            if (toptag.startsWith("router bgp ")
                ...
                traceVerbose(worker, "transformed => compacted "+toptag+" '"+line+"'");
                for (int c = n + 1; c < lines.length; c++) {
                    if (!lines[c].contains(match)) {
                        break;
                    }
                    if (lines[c].trim().startsWith("no ")) {
                        lines[c] = ""; // trim no neighbor *
                    }
                }
            }
    ...
    private String reorderOutput(NedWorker worker, String data) throws NedException {
        ...
        for (int n = 0; n < lines.length; n++) {
            String line = lines[n];
            //final String nextline = (n + 1 < lines.length) ? lines[n+1] : "";
            //final String nextline2 = (n + 2 < lines.length) ? lines[n+2] : "";
            if (line.isEmpty()) {
                sb.append("\n");
                continue;
            }
            ...
            /* segment-routing traffic-eng / no policy *
            if (line.startsWith("segment-routing traffic-eng")
                && nextline.startsWith(" no policy ")
                && "exit".equals(nextline2)) {
                traceVerbose(worker, "transformed => moved 'segment-routing traffic-eng /"+nextline+"' first");
                first.append(line+"\n"+nextline+"\nexit\n");
                n += 2; // skip nextline and nextline2
                continue;
            } else if (toptag.startsWith("segment-routing traffic-eng")
                && line.startsWith(" no policy ")) {
                traceVerbose(worker, "transformed => moved 'segment-routing traffic-eng /"+line+"' first");
                first.append(toptag+"\n"+line+"\nexit\n");
                continue;
            }
            */
    ...
    protected void enterConfig(NedWorker worker, String data) throws NedException, IOException, SSHSessionException {
        ...
        for (int n = 1; n < 10; n++) {
            ...
            final int hit = (int)res.getHit();
            switch (hit) {
            case 0:
                session.println("yes");
                continue;
            case 1:
                sleep(worker, (long)1000 * n, true);
            ...
            case 3:
                ...
                return;
            default:
                traceInfo(worker, "device error: hit="+hit+" text="+stringQuote(res.getText())
                          +" match="+stringQuote(expectGetMatch(res)));
                throw new NedException("config terminal: "+expectGetMatch(res));
            }
        }
        ...
        if (res != null) {
            ...
            throw new NedException("config terminal: timed out");
        }
    }
        ...
        try {
            ...
            while (retry && minutes > 0) {
                ...
                res = session.expect(new String[] {
                        ...
                        "\\A.*Set the configuration register boot bits to be non-zero",
                        ".*(error|ERROR|Invalid).*"
                    });
    ...
    private void print_line_wait(NedWorker worker, int cmd, String line, int retrying, String toptag, int num)
        ...
        for (int loops = 0; loops < 3; loops++) {
            ...
            // 10 - 'Old key: '
                ...
                if (this.oldConfigKey == null) {
                    ...
                    final String logpw = logIsDebug() ? this.oldConfigKey : "<hidden>";
        ...
        // no crypto pki server - special poll delete
        if (reply.contains("Certificate server 'remove server' event has been queued for processing")) {
            traceInfo(worker, "Waiting for 'crypto pki server' to be removed on device");
            final String pollLine = "do show run | i "+line.trim().replace("no ", "");
            for (int p = 1; p < configOutputMaxRetries + 1; p++) {
                sleep(worker, 1000, true);
                session.println(pollLine);
                session.expect(new String[] { Pattern.quote(pollLine) }, worker);
                String ret = session.expect(CONFIG_PROMPT, worker);
                if (ret.trim().isEmpty()) {
                    break;
                }
                traceInfo(worker, "'crypto pki server' still not deleted, retrying #"+p);
            }
        }
    }
    ...
    private int isCliRetry(String trimmed, String reply) {
        ...
        final String[] ignoreRetry = {
            ...
            "is in use and can not be deleted", // no key chain
        ...
        final String[] isRetry = {
            ...
            "One or more Xconnects are inoperable", // interface CEM0/2/6
    ...
    private boolean isCliError2(NedWorker worker, int cmd, String replyall, String reply, String line, String toptag) {
        ...
        if (line.startsWith("no transport ipv4 unicast interface") &&
            reply.contains("Given transport mode is not supported")) {
            traceInfo(worker, "ignoring delete warning");
            return false;
        }
    ...
    public void command(NedWorker worker, String cmdName, ConfXMLParam[] p) throws Exception {
        ...
        // internal - defaults delete
        else if ("defaults delete".equals(cmd)) {
            final String path = this.booleanOperPath;
            if (cdbOper.exists(path)) {
                reply.append("\nDeleted cached defaults.\n");
                cdbOper.delete(path);
            } else {
                reply.append("\nNo cached defaults to delete.\n");
            }
        }

        // internal - recompile-yang
        else if (cmd.startsWith("recompile-yang")) {
            try {
                Schemas.SchemaStore store = reloadSchemaFromFileBundle();
                if (cmd.endsWith(" global")) {
                    schemaStore = store;
                }
                List<String> warnings = store.getDiagnosticMessages();
                if (warnings.size() > 0) {
                    for (String w : warnings) {
                        reply.append("WARNING: " + w);
                    }
                } else {
                    reply.append("OK");
                }
            } catch (Exception e) {
                reply.append("ERROR: " + e.getMessage());
            }
        }

        // internal - show-running-dry
        else if (cmd.startsWith("show-running-dry")) {
            String res = modifyInput(worker, getConfig(worker));
            res = getConfigOnly(worker, res);
            parseConfig(worker, res, reply, false);
            if (reply.length() == 0) {
                reply.append("\nNo unknown configuration found\n");
            }
        }

        // internal - maapi-decrypt <password>
        else if (logIsVerbose() && logIsDebug() && cmd.startsWith("maapi-decrypt ")) {
        ...
        if (reply.length() == 0) {
            ...
            reply.append("  defaults delete - delete all cached defaults\n");
    ...
    private int intCmdCheckConfigTrace(NedWorker worker, File file, StringBuilder reply, StringBuilder uniqueSb)
        ...
        // Make NED able to parse traces written by versions older than 7.44
        String compat = sb.toString();
        compat = compat.replace("DONE SHOW", "done show");
        compat = compat.replace("BEGIN SHOW", "begin show");
        sb = new StringBuilder(compat);
        ...
        for (;;) {
            ...
            int end = sb.indexOf("\n-- done show", start);
            if (end < 0) {
                end = sb.indexOf("\n-- done extension exit handler", start);
            }
            if (end < 0) {
                reply.append("\n+++ "+name+":\n  Model: "+model+" Version: "
                             +version+"\n  INFO: Truncated trace, missing '-- done show|extension-handler'\n");
                this.totalFailed++;
                return 0;
            }
    ...
    protected void reconnectDevice(NedWorker worker) throws NedException {
        ...
        try {
            CliConnectorNedUtils.connectorReconnectDevice(this, worker);
        } catch (Exception e) {
    ...
    protected String shortpath(String path) {
        ...
        // oper data
        i = path.indexOf("/ios-op:");
        if (i > 0) {
            return path.substring(path.indexOf("/ios-op:")+8);
        }
    ...
    protected ArrayList<String[]> maapiGetObjects(NedWorker worker, int th, String path, int numLeaves)
        ...
        try {
            ...
            // Cleanup MaapiCursor to avoid late MAAPI_DESTROY_CURSOR message from garbage collector
            cr.destroy();
    ...
    private String injectData(NedWorker worker, String data, String[] entry, String dir) throws NedException {
        ...
        // append end of line (but not include it in match) to regex
        //regex += "(?:[\r])?(?=[\n])";
        regex += "(?:[\r])?[\n]";
        traceDev(worker, "injectdata regex = "+stringQuote(regex));
        ...
        if ("after-last".equals(where) || "after-topmode".equals(where)) {
            ...
            while (m.find()) {
                ...
                traceDev(worker, "   matched = "+stringQuote(replacement));
                try {
