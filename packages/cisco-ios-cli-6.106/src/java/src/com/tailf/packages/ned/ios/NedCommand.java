    ...
    private String doCommand(NedWorker worker, String cmd, boolean single) throws Exception {
        ...
        if (simulated != null) {
            ...
            for (String wait: cmd.split("\n")) {
                ...
                res = owner.session.expect(new String[] { this.errorPrompt, Pattern.quote(wait)},
                                           true, owner.writeTimeout, worker);
        ...
        if (!configMode && !help && cmd.startsWith("show ")) {
            ...
            res = owner.session.expect(new String[] { currentExecPrompt }, true, owner.writeTimeout, worker);
        ...
        while (true) {
            ...
            // If <abort> prompt, return what we got after 1 second maximum
            if (promptv != null && promptc < promptv.length) {
                final String answer = promptv[promptc];
                if ("<abort>".equals(answer) || "ABORT".equals(answer)) {
                    traceInfo(worker, "using | prompts answer #"+promptc+" "+stringQuote(answer));
                    try {
                        traceInfo(worker, "Aborting, waiting for last command reply (write-timeout 1000)");
                        res = owner.session.expect(cmdPatterns, true, 1000, worker);
                    } catch (Exception e) {
                        res = owner.session.expect(new Pattern[] { Pattern.compile(".*", Pattern.DOTALL) },true,0,worker);
                    }
                    traceVerbose(worker, "ExpectResult: hit="+res.getHit()+" text="+stringQuote(res.getText())
                                 +" match="+stringQuote(owner.expectGetMatch(res)));
                    reply.append(res.getText());
                    break;
                }
            }
